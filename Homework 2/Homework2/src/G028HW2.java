...
public class G028HW2 {
    public static void main(String[] args) throws IOException {
        ...
        // Parameters are: num_partitions, num_runs, flag, <path_to_file>
        if (args.length != 4) {
            throw new IllegalArgumentException("USAGE: num_partitions num_runs flag file_path");
        }

        // SPARK SETUP
        SparkConf conf = new SparkConf(true).setAppName("G028HW2");
        conf.set("spark.locality.wait", "0s");
        JavaSparkContext sc = new JavaSparkContext(conf);
        sc.setLogLevel("WARN");

        // INPUT READING
        // Read number of colors, number of repetitions and flag
        int C = Integer.parseInt(args[0]);
        int R = Integer.parseInt(args[1]);
        int F = Integer.parseInt(args[2]);

        // Read input file and subdivide it into 32 random partitions
        String filePath = args[3];
        System.out.println("Dataset = " + filePath);
        JavaRDD<String> docs = sc.textFile(filePath).repartition(32).cache();
        ...
        if (F == 0) {
            ...
            System.out.println("- Running time (average over " + R + " runs) = " + (int) averageTime + " ms");
        } else if(F == 1) {
            // ALGORITHM 2: MR_ExactTC
            System.out.println("Exact algorithm with node coloring");

            for (int i = 0; i < R; i++) {
                start = System.nanoTime();
                numTrianglesRuns[i] = MR_ExactTC(docs, C);
                end = System.nanoTime();
                timeRuns[i] = (end - start) / 1e6;
            }

            averageTime = DoubleStream.of(timeRuns).average().getAsDouble();
            System.out.println("- Number of triangles = " + numTrianglesRuns[R-1]);
            System.out.println("- Running time (average over " + R + " runs) = " + (int) averageTime + " ms");
        } else {
    ...
    public static long MR_ApproxTCwithNodeColors(JavaRDD<String> rawData, int C) {
        ...
        JavaPairRDD<Integer, Long> edges = rawData
                ...
                // R1 - SHUFFLE & GROUPING
                // Group pairs per color and return pairs (color, list of edges)
     ...
     */
    public static long MR_ExactTC(JavaRDD<String> rawData, int C) {
        // Definition of the parameters for computing the hash function hC
        final int p = 8191;
        Random rand = new Random();
        ...
        JavaPairRDD<Tuple3<Integer, Integer, Integer>, Long> edges = rawData
                ...
                .flatMapToPair((document) -> {  // document = a string: Ei = (u, v), with u, v vertices of the edge Ei
                    ...
                    for (String edge : edgeArray) {
                        ...
                        // Calculate the color of each of the two vertices of the edge
                        int hC_u = (int)(((a * u + b) % p) % C);
                        int hC_v = (int)(((a * v + b) % p) % C);

                        // Calculate the color triplet of the edge
                        for(int i = 0; i < C; i++) {
